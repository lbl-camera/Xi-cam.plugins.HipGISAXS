...
def showLattice(a, b, c, orders=7, basis=None, shape='Sphere', z0=0, xrot=0, yrot=0, zrot=0, **kwargs):
    vecs = latvec.latticevectors(a, b, c, kwargs['radius'], maxreps=100, repetitions=kwargs['repetitions'],
    ...
    for basisvec in basis:
        for vec in vecs:
            if shape == 'Sphere':
                addSphere(np.sum([list(map(np.add, vec, [0, 0, z0])), basisvec], axis=0), [kwargs['radius']] * 3, xrot,
                          yrot, zrot)
                linez = kwargs['radius']
            elif shape == 'Box':
                addBox(np.sum([list(map(np.add, vec, [0, 0, z0])), basisvec], axis=0),
                       [kwargs['length'], kwargs['width'], kwargs['height']], xrot, yrot, zrot)
                linez = kwargs['height']
            elif shape == 'Cylinder':
    ...
    if list(a) and list(b) and list(c):
        lines = latvec.latticelines(a, b, c, linez + z0, maxreps=100, repetitions=kwargs['repetitions'],
                                    scaling=kwargs['scaling'])
...
def addBox(center, scale, xrot, yrot, zrot):
    ...
    box = gl.GLMeshItem(vertexes=verts, faces=faces, color=(1, 0, 1, .3), shader='shaded', smooth=True,
                        glOptions='opaque')
...
def cylinder(rows, cols, radius=[1.0, 1.0], length=1.0, offset=False):
    ...
    top = ((np.arange(cols).reshape(cols, 1) + np.array([[0, 1, 0]])) % cols) * np.array([1, 1, 0]) + np.array(
        [[0, 0, len(verts) - 2]])
    down = ((np.arange(cols).reshape(cols, 1) + np.array([[0, 1, 1]])) % cols) * np.array([1, 1, 0]) + np.array(
        [[cols, cols, len(verts) - 1]])
    ...
    for feature in featuremanager.features:
        if type(feature) is customwidgets.layer:
            ...
            showLattice(map(float, feature.structure.LatticeA.value()), map(float, feature.structure.LatticeB.value()),
                        ...
                        shape=feature.Type.value(), radius=feature.Radius.Value.value(),
                        height=feature.Height.Value.value(),
                        width=feature.Width.Value.value(), length=feature.Length.Value.value(),
                        baseangle=feature.BaseAngle.Value.value(),
